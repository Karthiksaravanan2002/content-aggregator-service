...
type ContentItem struct {
	...
	PublishedAt  *time.Time  `json:"publishedAt,omitempty"` // When the content was published (nil if unknown)
	...
	ViewCount    int64       `json:"viewCount,omitempty"`   // Total view count if provided by source
	DurationSeconds int64    `json:"durationSeconds,omitempty"` // Duration in seconds (0 if not available)
...
type Extras struct {
	DurationSeconds *int64   `json:"durationSeconds,omitempty"` // Optional duration override when provider gives extra precision
