...
type FeatureAdapter func(ctx context.Context, req models.ProviderRequest) ([]models.ContentItem, error)


func (yt *YouTubeProvider) FetchFeature(ctx context.Context, req models.ProviderRequest, feature string,) ([]models.ContentItem, error) {
    adapter, ok := yt.adapters[feature]
    if !ok {
        return nil, errors.Unsupported(feature)
    }
    ...
    if !ok {
        return nil, fmt.Errorf("youtube: processing logic missing for feature %s", feature)
    }
